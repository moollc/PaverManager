...
(function() {
    ...
    function filterSuppliers(suppliers) {
        ...
        if (searchState.searchText) {
            var q = searchState.searchText.toLowerCase();
            filtered = filtered.filter(function(s) {
                var searchable = [s.name, s.contact_name, s.phone, s.email, s.address, s.notes].join(' ').toLowerCase();
                return searchable.indexOf(q) >= 0;
            });
        }

        // Filter by selected materials
        if (searchState.selectedMaterials.length > 0) {
            filtered = filtered.filter(function(s) {
                if (!s.materials || s.materials.length === 0) return false;
                var supKeys = s.materials.map(function(m) { return m.stockKey; });
                return searchState.selectedMaterials.some(function(k) { return supKeys.indexOf(k) >= 0; });
            });
        }
    ...
    function renderSupplierList() {
        ...
        var allSuppliers = loadSuppliers();
        var suppliers = filterSuppliers(allSuppliers);

        // Build search/filter UI
        var searchHtml = '<div style="margin-bottom:12px;">';
        searchHtml += '<input type="text" id="supplier-search" class="input-number" placeholder="Search suppliers..." value="' + escHtml(searchState.searchText) + '" style="text-align:left;width:100%;margin-bottom:8px;">';

        // Material filter chips
        searchHtml += '<div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:8px;">';
        STOCK_KEYS.forEach(function(k) {
            var isSelected = searchState.selectedMaterials.indexOf(k) >= 0;
            searchHtml += '<button class="supplier-mat-filter' + (isSelected ? ' active' : '') + '" data-key="' + k + '" style="font-size:0.7rem;padding:4px 8px;border-radius:12px;border:1px solid var(--border-color);background:' + (isSelected ? 'var(--accent-color)' : 'var(--bg-card)') + ';color:' + (isSelected ? '#fff' : 'var(--text-primary)') + ';cursor:pointer;">' + materialName(k) + '</button>';
        });
        searchHtml += '</div>';

        // Result count
        var totalText = suppliers.length === allSuppliers.length
            ? allSuppliers.length + ' supplier' + (allSuppliers.length !== 1 ? 's' : '')
            : suppliers.length + ' of ' + allSuppliers.length + ' suppliers';
        searchHtml += '<div style="font-size:0.75rem;color:var(--text-secondary);">' + totalText + '</div>';
        searchHtml += '</div>';

        if (suppliers.length === 0 && allSuppliers.length === 0) {
            container.innerHTML = searchHtml + '<div class="recipe-empty">No suppliers saved yet.</div>';
            wireSearchEvents();
            return;
        }

        if (suppliers.length === 0) {
            container.innerHTML = searchHtml + '<div class="recipe-empty">No suppliers match your search.</div>';
            wireSearchEvents();
            return;
        }
        ...
        suppliers.forEach(function(s, idx) {
            var realIdx = allSuppliers.indexOf(s);
            ...
            html += '<div class="recipe-row" data-sup-idx="' + idx + '" style="flex-direction:column;align-items:stretch;gap:4px;">' +
                '<div style="display:flex;align-items:center;gap:8px;">' +
                  ...
                  '<button class="recipe-btn recipe-btn-load sup-edit-btn" data-idx="' + realIdx + '">Edit</button>' +
                  '<button class="recipe-btn recipe-btn-del sup-del-btn" data-idx="' + realIdx + '">Delete</button>' +
                '</div>' +
    ...
    function wireSearchEvents() {
        ...
        document.querySelectorAll('.supplier-mat-filter').forEach(function(btn) {
            btn.addEventListener('click', function() {
                var key = btn.dataset.key;
                var idx = searchState.selectedMaterials.indexOf(key);
                if (idx >= 0) {
                    searchState.selectedMaterials.splice(idx, 1);
                } else {
                    searchState.selectedMaterials.push(key);
                }
                renderSupplierList();
            });
        });
    ...
    // DO NOT AUTO-FORMAT: HTML entities must be preserved
    function escHtml(s) {
